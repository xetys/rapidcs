...
function GenerateReport($combatResult, $raportInfo)
{
	...
	foreach($combatResult['rw'] as $Round => $RoundInfo)
	{
		...
		if(isset($RoundInfo['attack'], $RoundInfo['attackShield'], $RoundInfo['defense'], $RoundInfo['defShield']))
			$DATA['rounds'][$Round]['info']	= array($RoundInfo['attack'], $RoundInfo['attackShield'], $RoundInfo['defense'], $RoundInfo['defShield'],$RoundInfo['attackShoots'],$RoundInfo['defShoots']);
		else
			$DATA['rounds'][$Round]['info']	= array(NULL, NULL, NULL, NULL,NULL,NULL);
	}
	return $DATA;
}
